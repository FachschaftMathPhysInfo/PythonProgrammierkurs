...
#Theorie[
  ...
  // Zeile lesen, sofort ausführen, und auf die nächste warten. Genau das
    ...
    Erinnerst du dich noch an den Unterschied zwischen *Interpretierten* und *Compilierten* Programmiersprachen? Wir wollen uns jetzt mit einer nützlichen Konsequenz von Interpretierten Sprachen befassen.

    Die Interpretation erfolgt immer Zeile nach Zeile, Anweisung nach Anweisung. `python` fängt oben an und arbeitet das Programm ab. So hast du ja schon gesehen was passiert wenn wir eine Funktion aufrufen die erst später definiert wird.
  ...
  - Du tippst ```python exit()``` und drückst Enter.
...
#Praxis[
  *Aufgabe 1:*
  ...
  + Tipp die folgenden Zeilen einzeln ein, drücke nach *jeder* Zeile Enter
    und schau, was passiert, bevor du weitermachst:
    ...
    10 / 3
    ```
  + Verlasse das REPL wieder mit ```python exit()```.
...
#Theorie[
  ...
  Skript passiert das *nicht*, dort musst du explizit ```python print()``` verwenden,
...
#Praxis[
  *Aufgabe 2:*
  ...
  + Tipp die vier Zeilen aus `repl_vs_skript.py` einzeln ins REPL ein.
...
#Theorie[
  ...
  ```
    ...
    Das bleibt solange bestehen, bis du die entsprechende Klammer schließt.
    Danach kommst du zu dem normalen Prompt zurück und Python interpretiert die Eingabe.

    Man kann so auch Einrückungen im Repl nutzen. Wenn du beispielsweise eine ```python for``` schleife anfängst gelangst du ebenfalls zu dem anderen Prompt. Du beendest diesen Modus, wenn du eine Leerzeile eingibst.
  ...
  // Ergebnissen den Wert einer Variable noch einmal einzeln anzeigen lassen
    ...
    *Die help Funktion*
    ...
    Das REPL bietet uns eine Funktion an, die es in normalem Python so nicht gibt, die ```python help()``` Funktion. Wenn du sie einfach so aufrufst, landest du in einer interaktiven Hilfe-Anwendung. Alternativ, wenn du weißt wonach du suchst, kannst du auch direkt Argumente an help geben.

    *Das REPL vergisst #strike("nicht")*

    Das REPL hat keine Speicherfunktion. Wenn du dort ein tolles Programm geschrieben hast, musst du es per Hand rausschreiben, ansonsten geht es irgendwann verloren.
...
#Spiel[
    Probier mal aus eine der letzten Aufgaben im REPL zu lösen. Ist das einfacher oder schwieriger?
]
